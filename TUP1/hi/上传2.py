...
from flask import Flask, request, redirect, url_for, flash, send_file, session, jsonify
import boto3
...
<body>
    ...
    <script>
        ...
        function generateShareLink() {
            ...
            formData.append('key', document.getElementById('shareKey').value);
            formData.append('expires_in', form.elements['expires_in'].value);
            formData.append('expires_unit', form.elements['expires_unit'].value);
            
            fetch('/generate-share-url/{{ bucket_name }}', {
                method: 'POST',
                body: formData
            })
            ...
            .then(data => {
                if (data.success) {
                    ...
                    document.getElementById('expiryInfo').textContent = '链接将在 ' + data.expires_at + ' 过期';
                ...
                <div class="file-item">
                    ...
                    <div class="file-actions">
                        <button class="btn btn-outline-info btn-sm" data-bs-toggle="modal" data-bs-target="#shareModal" 
                                data-key="{folder['prefix']}" data-filename="{folder['name']}">
            ...
            for obj in objects:
                ...
                <div class="file-item">
                    ...
                    <div class="file-actions">
                        ...
                        <button class="btn btn-outline-info btn-sm" data-bs-toggle="modal" data-bs-target="#shareModal" 
                                data-key="{obj['key']}" data-filename="{obj['name']}">
                            <i class="bi bi-share"></i>
                        </button>
        ...
        <div class="modal fade" id="shareModal" tabindex="-1">
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <form id="shareForm">
                        <div class="modal-body">
                            <input type="hidden" id="shareKey" name="key">
                            <div class="mb-3">
                            ...
                            <div class="mb-3">
                                ...
                                <div class="input-group">
                                    <input type="number" class="form-control" name="expires_in" value="60" min="1" required>
                                    <select class="form-select" name="expires_unit">
                            ...
                            <div class="mb-3" id="shareResult" style="display: none;">
                                <label class="form-label">分享链接</label>
                                <div class="input-group">
                                    <input type="text" class="form-control" id="shareUrl" readonly>
                                    <button type="button" class="btn btn-outline-secondary" onclick="copyShareUrl()">
                        ...
                        <div class="modal-footer">
                            ...
                            <button type="button" class="btn btn-primary" onclick="generateShareLink()">生成链接</button>
                        </div>
                    </form>
                </div>
            </div>
...
@app.route('/generate-share-url/<bucket_name>', methods=['POST'])
def generate_share_url(bucket_name):
    key = request.form.get('key')
    expires_in = int(request.form.get('expires_in', 60))
    expires_unit = request.form.get('expires_unit', 'minutes')
    
    # 将时间转换为秒
    if expires_unit == 'minutes':
        expires_seconds = expires_in * 60
    elif expires_unit == 'hours':
        expires_seconds = expires_in * 3600
    elif expires_unit == 'days':
        expires_seconds = expires_in * 86400
    else:
        expires_seconds = expires_in * 60  # 默认分钟
...
@app.route('/share/<bucket_name>/<path:key>')
def share_file(bucket_name, key):
    """分享文件代理路由"""
    # 获取过期时间参数
    expires = request.args.get('expires')
    
    if not expires:
        return redirect('/share-expired')
    
    # 检查链接是否过期
    try:
        expires_time = datetime.fromtimestamp(float(expires))
...
def share_expired():
    ...
    <body>
        <div class="container">
            ...
            <h1>链接已失效</h1>
            <p>此分享链接已超过有效期，无法访问文件。</p>
            <p>请联系文件分享者获取新的有效链接，或直接联系系统管理员。</p>
